...
\begin{corollary}
	\cite[89]{Aigner2007}
	\label{oneColor} Let $|R| = r = 2$. Let $x_1 = x_{\text{white}} = x$ and
...
%--------------------------------------------------------------------------
\begin{example}
	We use Polya's theorem to describe the pattern inventory of 3-bead necklaces under the action of $S_3$. Let the colors be $A$ and $B$. Then
	\begin{equation}
		\begin{align}
...
\begin{example}
	...
	transposition by setting our set of colors to be $R = \{ 0, 1 \}$. We then disregard
	one of the colors, say zero, by setting $w(0) = 1$ and $w(1) = C$. The group of
	...
	\begin{equation}
		\begin{align}
			...
			2 x_6^{2} + 4 x_12^1 \right) \\
			& = \frac{1}{12} \left[ (1 + C)^{12} + (1 + C^2)^{6} + 2 (1 + C^3)^{4} +
			2 (1 + C^4)^{3} + 2 (1 + C^6)^{2} + 4 (1 + C^{12}) \right] \\
			& = C^{12} + C^{11} + 6 C^{10} + 19 C^9 + 43 C^8 + 66 C^7 + \cdots \\
			& \qquad \qquad \cdots +
			80 C^6 + 66 C^5 + 43 C^4 + 19 C^3 + 6 C^2 + C^1 + C^0 \enspace.
		\end{align}
	\end{equation}
	In words, we have 43 tetrachords and 19 trichords that are transpositionally equivalent.
\end{example}
