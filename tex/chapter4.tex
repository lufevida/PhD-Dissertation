...
\addtocounter{enumi}{2}
\item If the row is retrograde inverse-invariant, line 30 iterates over the size of the row, filling the first half of the retrograde map in line 31. Note that if the row is retrograde inverse-invariant, the first and second halves of the retrograde map are reversed, that is, line 31 does the same computation that line 28 does.
\addtocounter{enumi}{2}
...
\addtocounter{enumi}{3}
\item Line 37 iterates a second time over the entire size of the row class to set the values in the retrograde inverse map. Line 37 tests if the entry corresponds to the $T_0$ or the $T_0I$ transforms of the row, in which case line 38 assigns these indices such that $T_0 \mapsto T_0I$ and $T_0I \mapsto T_0$. If not, then line 39 the other indices such that $T_0 \mapsto T_0I$.
\end{enumerate}

\begin{example}
Let $r$ be a retrograde-invariant row with size $n = 6$. Then the row class array computed by Listing~\ref{getRowClass} will contain a sequence of transforms of $r$ equal to $C = \{T_0, \cdots, T_5, T_0I, \cdots, T_5I\}$. Let $\tilde{C} = \{0, \cdots, 11\}$ be the sequence of indices of $C$, such that $T_0 \mapsto 0, \cdots, T_5I \mapsto 11$ is a mapping from $C$ to $\tilde{C}$. Then the retrograde map of the row class of $r$ will be the sequence of indices
\begin{equation}
	R(\tilde{C}) = \{3, 4, 5, 0, 1, 2, 9, 10, 11, 6, 7, 8\} \enspace.
\end{equation}
Since $r$ is retrograde-invariant, the sequence $\{0, \cdots, 5\}$ of indices of $r$ maps to itself under the retrograde operator, that is, $R$ is the permutation $(0 \; 5) \; (1 \; 4) \; (2 \; 3)$ when seen as a permutation of indices. Since $r = RT_x(r)$ by assumption, $Tx$ must be a transposition that, seen as a permutation of pitch-classes, in which all cycles are 2-cycles. This is only possible if $x = 3$. Since $T_3 = (0 \; 3) \; (1 \; 4) \; (2 \; 5)$, the array given by $R(\tilde{C})$, when seen as a permutation of indices of the row class, is in fact the map
\begin{equation}
	T_0 \mapsto T_3, \cdots, T_5 \mapsto T_2, T_3I \mapsto T_0I, \cdots, T_5I \mapsto T_2I \enspace.
\end{equation}
If, instead, $r$ is retrograde inverse-invariant, let $C$ and $tilde{C}$ be as above. Then the retrograde inverse map of the row class of $r$ will be the sequence of indices
\begin{equation}
	RI(\tilde{C}) = \{11, 6, 7, 8, 9, 10, 1, 2, 3, 4, 5, 0\} \enspace.
\end{equation}
