...
%--------------------------------------------------------------------------
\begin{corollary}
    Let $r$ be a row, let the problem size be equal to $m$, and regard the top vector as the direct product of $m$ copies of $G$, the group of $RT_nI$-transforms of $r$. If a solution exists for a combination of top, side and square, then right-multiplying the solution by an element of $G$ represents a group action that induces an equivalence class of solutions. In particular, this equivalence class preserves the structure of the semi-magic square.
    \begin{proof}
    The action of $G$ on itself by right multiplication is a permutation of $G$. Similarly, if $G_m$ is the direct product of $m$ copies of $G$, the action of $G$ on $G_m$ induces a permutation in each component of $G_m$. An element of $G_m$ is a top, and each component of a top is a column. If $S$ is a solution, that is, a combination matrix of self-derivation, then the action of $G$ on the top may be extended to each component of the side vector as well, since the side vector may also be construed as an element of $G_m$. By the proof of Th.~\ref{topSquareSideTheorem}, each derived row in the side vector fits uniquely under the top, since $S$ is a solution. For every column of the top, the permutation induced by the action of $G$ is the same permutation induced on the partition of the side row that fits under that top column. In particular, the semi-magic square and the alignment between the top column entries and the side partition entries are unchanged by this permutation. The same reasoning applies to every other side row, yielding the result.
    \end{proof}
\end{corollary}
