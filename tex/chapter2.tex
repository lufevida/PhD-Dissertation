...
\begin{corollary}
    ...
    \begin{proof}
    The action of $G_r$ on itself by right multiplication is a permutation of $G_r$. Similarly, if $G$ is the direct product of $m$ copies of $G_r$, the action of $G_r$ on $G$ is a permutation of each entry in $G$. An element of $g$ is a top. If $S$ is a solution, that is, a combination matrix of self-derivation, then the action of $G_r$ on the top may be extended to each entry in the side vector as well, since the side vector may also be construed as an element of $G$. By Th.~\ref{topSquareSideTheorem}, each derived row in the side vector fits uniquely under the top, since $S$ is a solution. For every column of the top, the permutation induced by the action of $G_r$ is the same permutation induced on the partition of the side row that fits under that top column. In particular, the semi-magic square and the alignment between the top column entries and the side partition entries are unchanged by this permutation. The same reasoning applies to every other side row, yielding the result.
    \end{proof}
\end{corollary}
